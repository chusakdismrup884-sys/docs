---
title: Set up decision model online evaluators
sidebarTitle: Decision model
description: Evaluate production traces in LangSmith with SemIf or Jev as the judge, and record each question's typed answer as its own feedback key.
---

[Online evaluations](/langsmith/evaluation-concepts#online-evaluations) provide real-time feedback on your production traces. Decision model online evaluators use a decision model, such as SemIf or Jev, as the judge in place of an LLM. To use an LLM as the judge, see [Set up LLM-as-a-judge online evaluators](/langsmith/online-evaluations-llm-as-judge).
...
[Decision models](/langsmith/llm-gateway-decision-models) answer structured questions about text and return typed answers, such as a probability, a selected option, or a score, rather than free-form text. When you use a decision model as the judge, LangSmith records the answer to each question you define under its own feedback key, so you do not need to write an output schema or parse free-text reasoning.
...
The evaluator evaluates incoming runs or threads that match its filter, and each question's answer appears under its own feedback key.
