...
Pinning both keeps the global `mda` binary and the project's own dependency on one version. For the rest of the flags, see [Deploy projects](/langsmith/python/managed-deep-agents-cli#deploy-projects), and for deploying without the agent flags, see [Deploy a Managed Deep Agent](/langsmith/python/managed-deep-agents-deploy).
