...
A TypeSafe-compatible endpoint is a [model configuration](/langsmith/model-configurations) that points a decision model evaluator at any server that implements the [TypeSafe System One API](https://docs.typesafe.ai). Use one to call TypeSafe models through another provider, such as OpenRouter, or to call a decision model you host yourself. For LLMs, see [Connect to an OpenAI compliant model provider/proxy](/langsmith/custom-openai-compliant-model).
