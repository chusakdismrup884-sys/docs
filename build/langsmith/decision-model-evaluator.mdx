...
Decision model evaluators support TypeSafe models, such as Jev, with bring-your-own-key (BYOK), and SemIf through the LangSmith Gateway. You define the state and questions in the evaluator itself. Decision model evaluators cannot load a prompt from the Prompt Hub or use a custom output schema. They also do not support [few-shot examples](/langsmith/create-few-shot-evaluators), and no [evaluator templates](/langsmith/llm-as-judge#evaluator-templates) use a decision model. To reuse a decision model evaluator on another dataset or tracing project, select **Attach an existing evaluator** in the **Add Evaluator** panel.
