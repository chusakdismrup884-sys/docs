...
description: Set up a LangSmith evaluator that uses SemIf or Jev as the judge, with noul, choice, and score questions that each return typed feedback.
---
...
1. In the [LangSmith UI](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-decision-model-evaluator), open the **Evaluators** tab of a dataset or tracing project and click **+ Evaluator**.
1. In the **Add Evaluator** panel, select **LLM-as-a-Judge Evaluator** under **Create from scratch**.
...
- For SemIf, select **LangSmith Gateway** as the provider, then select a SemIf model.
- For Jev, select **TypeSafe** as the provider, then select a Jev model. If you stored your key under a name other than `TYPESAFE_API_KEY`, enter that name in **API Key Name**.
...
A TypeSafe-compatible endpoint points the evaluator at any server that implements the [TypeSafe System One API](https://docs.typesafe.ai), such as OpenRouter or a decision model you host yourself. It has three settings:
...
1. In **Model Configuration**, for **Provider**, select **TypeSafe Compatible Endpoint**. It appears under **Other**.
...
For example base URLs, see [Connect to a TypeSafe-compatible model provider](/langsmith/typesafe-compatible-model).
...
Once you are finished configuring, save the evaluator.
