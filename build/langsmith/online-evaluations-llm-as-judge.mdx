...
You can use a decision model, such as SemIf or Jev, as the judge in place of a chat model. Each question you define returns a typed answer, such as a probability, a selected option, or a score, and LangSmith records each one under its own feedback key. For supported models, question types, and setup, see [How to define a decision model evaluator](/langsmith/decision-model-evaluator).
