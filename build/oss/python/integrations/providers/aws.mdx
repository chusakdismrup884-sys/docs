...
The [AWS Bedrock Mantle API](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-responses-api.html#bedrock-mantle-responses) exposes frontier models through the OpenAI and Anthropic Messages protocols. `ChatOpenAIMantle` and `ChatAnthropicMantle` are the recommended interfaces for accessing these models.
...
`ChatOpenAIMantle` inherits from [`BaseChatOpenAI`](/oss/python/integrations/chat/openai), so tool calling, structured output, streaming, and multimodal support are all inherited. See the [AWS Mantle OpenAI-compatible API docs](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-responses-api.html#bedrock-mantle-responses) for supported models.

`ChatAnthropicMantle` inherits from [`ChatAnthropic`](/oss/python/integrations/chat/anthropic) and targets the Anthropic Messages endpoint on Mantle. See the [AWS Mantle Anthropic API docs](https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-anthropic-claude-messages.html) for supported models.
